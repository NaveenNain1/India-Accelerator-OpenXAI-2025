...
const ThemeOutlet = ({ user,children}) => {
  ...
  const [showBackArrow, set_showBackArrow] = useState(false);

  useEffect(() => {
    if (page) {
      set_pageTitle(page.pageTitle || "");
      set_showBackArrow(page.showBackArrow || false);
    }
  }, [page]);
  ...
  const getSidebarNavLinkClass = (path) => 
    ...
    <div className="flex h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 font-sans">
      ...
      <aside className="hidden md:flex md:flex-col md:w-72 bg-white/80 backdrop-blur-sm border-r border-white/20 shadow-xl rounded-r-3xl" style={{ zIndex: 10 }}>
        ...
        <nav className="flex-1 px-6 py-6 space-y-3">
          {menuItems.map((item) => (
            <Link   key={item.path}  href={item.path} className={getSidebarNavLinkClass(item.path)}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-sm border-t border-white/20 shadow-xl rounded-t-3xl" style={{ zIndex: 50 }}>
        ...
        {menuItems.map((item) => {
          ...
          return (
          <Link   key={item.path}  href={item.path} className={getBottomNavLinkClass(item.path)}>
